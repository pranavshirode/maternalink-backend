...
import { exercises, ExerciseCategory } from '../../application/data/exercises';
...
function buildCategory(key: string): ExerciseCategory {
  return {
    category: key,
    ...(exercises[key] as any),
  };
}

function buildSensorSummary(data: any) {
  return {
...
function buildResponse(categoryKeys: string[], reasons: string[], data: any) {
...
export class ExerciseController {
  public static async getRecommendations(req: Request, res: Response) {
    try {
      ...
      if (heartRate >= THRESHOLDS.heartRate.high || temperature >= THRESHOLDS.temperature.high) {
        ...
        return res.json(buildResponse(['recovery_rest'], [reason], data));
      }
      ...
      if (activityIndex <= THRESHOLDS.activity.veryLow) {
        recommendations.push('low_activity', 'stretching');
        ...
        recommendations.push('stretching', 'low_activity');
      ...
      return res.json(buildResponse(recommendations, reasons, data));
    } catch (err: any) {
  ...
  public static async getAllExercises(req: Request, res: Response) {
    return res.json({
      success: true,
